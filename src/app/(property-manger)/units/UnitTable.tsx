"use client";
import DeleteModal from "@/components/DeleteModal";
import { API_URL, ENDPOINTS } from "@/constants/ApiUrls";
import { fetchData } from "@/lib/db_operations";
import { Unit } from "@/lib/types";
import { useAtom, useSetAtom, useAtomValue } from "jotai";
import { useEffect, useState, useCallback } from "react";
import 'react-data-grid/lib/styles.css';
import toast from "react-hot-toast";
import { unit, units, pageTitle } from "./unitAtoms"; // Import pageTitle
import PageHead from "@/components/PageHead";
import UnitForm from "./UnitForm";
import { UnitStatus } from "@/lib/status";
import { DataTable } from "@/components/reports/data-table";
import { getColumns } from "./column"; // Import getColumns

const UnitTable = ({ dataItems }: { dataItems: Unit[]; }) => {
    const [openDeleteModal, setDeleteModal] = useState(false);
    const [deleteId, setDeleteId] = useState<number | undefined>(undefined);
    const [data, setData] = useAtom(units);
    const setEditUnit = useSetAtom(unit);
    const pageHead = useAtomValue(pageTitle); // Use pageTitle atom
    const [openUnitEdit, setOpenUnitEdit] = useState(false);
    const [openAddUnit, setAddUnit] = useState(false);

    // Fetch data on component mount
    useEffect(() => {

        setData(dataItems);

    }, [setData, dataItems]);

    // Edit handlers
    const handleEdit = useCallback((data: Unit) => {
        setEditUnit(data);
        setOpenUnitEdit(true);
    }, [setEditUnit]);

    const handleCloseEdit = useCallback(() => {
        setEditUnit({
            unitNumber: "",
            status: UnitStatus.Available,
            apartmentId: 0,
            id: undefined,
        });
        setOpenUnitEdit(false);
    }, [setEditUnit]);

    // Add handlers
    const handleOpenAddForm = useCallback(() => {
        setAddUnit(true);
    }, []);

    const handleCloseAddForm = useCallback(() => {
        setAddUnit(false);
    }, []);

    // Delete handlers
    const handleDelete = useCallback((data: Unit) => {
        setDeleteId(data.id);
        setDeleteModal(true);
    }, []);

    const handleCloseDelete = useCallback(() => {
        setDeleteModal(false);
    }, []);

    const deleteItem = useCallback(async () => {
        if (deleteId === undefined) return;
    
        try {
            const response = await fetchData(`${API_URL.EXTERNAL_API_URL}${ENDPOINTS.UNIT}/${deleteId}`, {
                method: "DELETE",
            });
    
            // Handle 204 No Content response
            if (response === null) {
                // For 204 responses, assume the deletion was successful
                setData((prevData) => prevData.filter((unit) => unit.id !== deleteId));
                toast.success(`Successfully deleted ${pageHead}`);
            } else {
                // For responses with a body, use the returned data (if applicable)
                setData((prevData) => prevData.filter((unit) => unit.id !== deleteId));
                toast.success(`Successfully deleted ${pageHead}`);
            }
        } catch (err: unknown) {
            if (err instanceof Error) {
                toast.error(`Failed to delete ${pageHead}: ${err.message}`);
            } else {
                toast.error(`Failed to delete ${pageHead}`);
            }
        } finally {
            handleCloseDelete();
        }
    }, [deleteId, setData, handleCloseDelete, pageHead]);
    return (
        <>
            {openAddUnit && <UnitForm handleClose={handleCloseAddForm} head="Add " edit={false} />}
            {openUnitEdit && <UnitForm handleClose={handleCloseEdit} head="Edit " edit={true} />}

            <PageHead title={pageHead} handleAdd={handleOpenAddForm} />
            {openDeleteModal && <DeleteModal handleDelete={deleteItem} close={handleCloseDelete} />}
            <DataTable columns={getColumns(handleEdit, handleDelete)} data={data} />
        </>
    );
};

export default UnitTable;